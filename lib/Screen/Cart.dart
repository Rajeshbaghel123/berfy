...
import 'package:berfy/Screen/PromoCode.dart';
...
class StateCart extends State<Cart> with TickerProviderStateMixin {
  ...
  void startPaymentPhonePe({required String orderId}) async {
    try {
      var uuid =
          const Uuid().v4().substring(0, 30); // Generates a random UUID (v4)
      String amt = ((usedBal > 0
                  ? totalPrice
                  : isStorePickUp == "false"
                      ? (totalPrice + delCharge)
      ...
      final jsonData = {
        ...
        "amount": num.parse(amt).toInt(),
        "redirectUrl": phonePeEndPointUrl,
        "redirectMode": "REDIRECT",
        "callbackUrl": phonePeEndPointUrl,
        "mobileNumber": context.read<UserProvider>().mob,
      ...
      PhonePePaymentSdk.startPGTransaction(
              ...
              pgHeaders,
              apiEndPoint,
              /* "com.phonepe.app.preprod"*/ Platform.isAndroid
                  ? packageName
                  : iosPackage)
          .then((response) async {
        if (kDebugMode) {
        ...
        if (response != null) {
          ...
          if (status == 'SUCCESS') {
            ...
            Map<String, dynamic> result =
                await updateOrderStatus(orderID: orderId, status: PLACED);
            await addTransaction(uuid, orderId, SUCCESS, status, true);
      ...
      child: InkWell(
        ...
        onTap: () async {
          ...
          await Navigator.push(
              ...
              CupertinoPageRoute(
                  builder: (BuildContext context) =>
                      Payment(updateCheckout, msg))).then((value) {
