...
import 'package:berfy/Screen/Dashboard.dart';
import 'package:device_info_plus/device_info_plus.dart';
import 'package:berfy/Helper/Session.dart';
import 'package:berfy/Model/Order_Model.dart';
import 'package:berfy/Screen/Review_Gallary.dart';
import 'package:berfy/Screen/Review_Preview.dart';
...
class StateOrder extends State<OrderDetail>
  ...
  late Timer _timer;
  late DateTime _startTime; // 30 minutes in seconds
  ...
  Future<void> _playAnimation(AnimationController animationController) async {
    try {
  ...
  Widget build(BuildContext context) {
    ...
    _myOrderId = model.id!;
    _startTime = DateTime.parse(model.dateTime!);
    _startTimer();
            ...
            child:
              ...
              Padding(
                ...
                child: Row(
                  ...
                  children: [
                    Text("Original ${getTranslated(context, 'PRICE_LBL')!} :",
...
/*  DwnInvoice() async {
              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                    ...
                    _timerRemaining != "" && _timerRemaining != "Window closed."
                        ? InkWell(
                            onTap: () {
                ...
                child: Form(
                  ...
                  child:
                    ...
                    return Column(mainAxisSize: MainAxisSize.max, children: [
                      bottomSheetHandle(context),
                      bottomsheetLabel("CONFIRM_ORDER", context),
                      setNameField(),
                      saveButton(
  ...
  Future<bool> validateAndSave(GlobalKey<FormState> key) async {
    ...
    if (form.validate()) {
      ...
      setUpdateUser(_myOrderId, nameController.text);
      return true;
    }
    return false;
  }

  Future<void> setUpdateUser(String id, username) async {
    var apiBaseHelper = ApiBaseHelper();
    var data = {ORDER_ID: id, STATUS: username};

    print("profile data****$data");
    final result = await apiBaseHelper.postAPICall(getUpdateOrderNoteApi, data);
    ...
    if (!error) {
      ...
      Navigator.pushAndRemoveUntil(
          ...
          (route) => false);
      setSnackbar("Updated Successfully", context);
    } else {
      setSnackbar(msg!, context);
    }
  ...
  void _startTimer() {
    ...
    print("Ticket window opens at: ${startTime}");
    print("Ticket window closes at: ${endTime}");

    // if (endTime.isAfter(DateTime.now())) {
    //   print(
    //       "Ticket window cannot be started as end time is before current time.");
    // } else {
    Timer.periodic(Duration(seconds: 1), (Timer timer) {
      Duration remainingTime = endTime.difference(DateTime.now());
      if (remainingTime.inSeconds <= 0) {
        timer.cancel();
        _timerRemaining = "Window closed.";
      } else {
        _timerRemaining = _formatDuration(remainingTime) + " Min";
      }
      setState(() {});
      print(_timerRemaining);
    });
    // }
  }
  ...
  Widget setNameField() => Container(
        ...
        child: Padding(
          ...
          child: TextFormField(
            ...
            decoration: InputDecoration(
                label: Text(getTranslated(context, "ORDER_ID_LBL")!),
                fillColor: Theme.of(context).colorScheme.white,
                border: InputBorder.none),
            validator: (val) => validateUserName(
                val!, "Order ID is Required", "Order ID is Required"),
          ),
        ),
      );
