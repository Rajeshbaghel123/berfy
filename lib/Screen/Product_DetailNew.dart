...
import 'package:berfy/Screen/Cart.dart';
import 'package:http/http.dart' as http;
import 'package:berfy/Screen/FaqsProduct.dart';
import 'package:berfy/Screen/ListItemCompare.dart';
...
import 'package:berfy/Screen/ReviewList.dart';
import 'package:berfy/main.dart';
...
class StateItem extends State<ProductDetail> with TickerProviderStateMixin {
  ...
  Future<void> createDynamicLink(Product data) async {
    String shortenedLink = await generateShortDynamicLink(data);
          ...
          Navigator.push(
            ...
            CupertinoPageRoute(
                builder: (context) => Login(
                      classType: Cart(fromBottom: false),
                      isPop: true,
                      isRefresh: true,
                    )),
          );
  ...
  Future<void> placeOrder(String? tranId) async {
    ...
    if (_isNetworkAvail) {
      ...
      String? payVia;
      String payMethod = getTranslated(context, 'COD_LBL')!;
      payVia = "COD";
      ...
      try {
        ...
        request.fields[TOTAL] = price.toString();
        request.fields[FINAL_TOTAL] = price.toString();

        request.fields[TAX_PER] = "";
        request.fields[PAYMENT_METHOD] = payVia!;

        request.fields[ISWALLETBALUSED] = "0";
        request.fields[WALLET_BAL_USED] = "0";
        request.fields[ORDER_NOTE] = "";

        request.fields[ADD_ID] = "";
        request.fields[DELIVERY_TIME] = 'Anytime';
        ...
        request.fields[LOCAL_PICKUP] = "1";

        // if (IS_LOCAL_PICKUP == "1") {
        //    = isStorePickUp == "true" ? "1" : "0";
        // }
            ...
            Navigator.pushAndRemoveUntil(
                ...
                CupertinoPageRoute(
                    builder: (BuildContext context) => OrderSuccess(
                          url: productData!.productIdentity!,
                        )),
...
/* Future getProductFaqs() async {
  ...
  _Write_Full3(Text1, Text2, Text3,
  ...
  _onlyprice(pos, from, Product data) {
    Product model = data;
    ...
    String dataprice = model.isSalesOn == "1"
        ? getPriceFormat(
            context, double.parse(model.prVarientList![pos].saleFinalPrice!))!
        : '${getPriceFormat(context, price)!} ';
    ...
    return data.desc!.isNotEmpty
        ? Padding(
            ...
            child: HtmlWidget(
              ...
              textStyle: TextStyle(
                  fontSize: 11, color: Theme.of(context).colorScheme.fontColor),
            ),
          )
        : const SizedBox();
  }
  ...
  late Product data_circulated;

  _showContent() {
    if (productData != null && isLoadedAll) {
      Product data = productData!;
      data_circulated = productData!;
      // double originalPrice = double.parse(data.prVarientList![_oldSelVarient].price!);
      // double pricepay = double.parse(data.prVarientList![_oldSelVarient].saleFinalPrice!);
      // double discount = originalPrice - pricepay;
      // double cashback = data.gurantee! == "" ? 0 : double.parse(data.gurantee!);
      // double actualpricepay = pricepay - cashback;
      // double earning = double.parse(data.prVarientList![_oldSelVarient].earn_info_w!);
      // double totalreceive =  actualpricepay + earning;
            ...
            child: CustomScrollView(
              ...
              SliverAppBar(
                ...
                flexibleSpace: FlexibleSpaceBar(
                  background: 1 == 1 ? _slider(data) : Container(),
                ),
              ),
              SliverList(
                            ...
                            child: Column(
                              ...
                              children: [
                                Card(
                                  elevation: 1,
                                  color: colors.whiteTemp,
                                  surfaceTintColor: colors.whiteTemp,
                                  child: Container(
                                ...
                                Padding(
                                  ...
                                  child: Row(
                                    ...
                                    children: [
                                      networkImageCommon(
                                          sliderList[0]!, 70, false,
                                          width: 70, height: 70),
                                      Expanded(flex: 1, child: _title(data)),
                                      _brandName(data)
                                    ],
                                  ),
                                ),
                                  ...
                                  child: Card(
                                    ...
                                    child: Container(
                                      ...
                                      child: Column(
                                        ...
                                        children: [
                                          _Write_Full3("You'll Spent", "", ""),

                                          _price(_oldSelVarient, true, data),

                                          // _Write_Full_Large(
                                          //     _onlyprice(
                                          //         _oldSelVarient, false, data),
                                          //     ""),
                                          ...
                                          //Double asd = double.parse(data.prVarientList![_oldSelVarient].disPrice!);
                                          _Write_Full3(
                                              getPriceFormat(
                                                  context,
                                                  double.parse(data
...
/* horizontal: 20,*/
                      ...
                      child: GestureDetector(
                        onTap: () {
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              content: Text(model.shortDescription! != ""
                                  ? 'Copied to clipboard'
                                  : "Address is Blank"),
                            ),
                          );
  ...
  _brandName(Product data) {
    ...
    return model.brand != ""
        ? Card(
            color: colors.whiteTemp,
            shadowColor: colors.whiteTemp,
            elevation: 5,
            child: Row(
              children: [
                ClipRRect(
                    borderRadius: BorderRadius.circular(12.0),
                    child: networkImageCommon(appUrl + model.brand!, 55, false,
                        height: 55, width: 55)),
                // Text(
