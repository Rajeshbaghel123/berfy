...
import 'package:berfy/Screen/HomePage.dart';
...
class Favorite extends StatefulWidget {
  ...
  const Favorite({Key? key,required this.fromBottom}) : super(key: key);
    ...
    return Scaffold(
      backgroundColor: colors.blackTemp,
        appBar:  widget.fromBottom
            ? null
            : getAppBar(getTranslated(context, 'FAVORITE')!, context),
    ...
    return Selector<FavoriteProvider, Tuple2<bool, List<Product>>>(
        builder: (context, data, child) {
          return data.item1
              ? shimmer(context)
              : data.item2.isEmpty
                  ? Center(child: Text(getTranslated(context, 'noFav')!,style: TextStyle(color: colors.whiteTemp),))
                  : RefreshIndicator(
                      color: colors.primary,
